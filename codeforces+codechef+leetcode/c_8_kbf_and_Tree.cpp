...
void dfs(int node, int parent, int &root, int val, vvp &adj, vector<set<pair<int,int>>> &vv ){

    for(auto adjNode:adj[node]){

        if(adjNode.ff != parent)
        {
            pair<int, int> p = {min(root, adjNode.ff), max(root, adjNode.ff)};
            int x = val ^ adjNode.ss;

            if(vv[x].empty()){
                vv[x].insert(p);

            } else if(*vv[x].begin() != p){
                ans = x;
                vv[x].insert(p);
                return;
            }
            dfs(adjNode.ff, node, root, x, adj, vv);
        }
...
void prakhar() {   
    ...
    vector<set<pair<int, int>>> vv(1 << 21);
    ans = -1;
    for (int i = 1; i <= n; i++)
    {
            dfs(i, -1, i, 0, adj, vv);
