...
void dfs(vector<vector<int>>& image, vector<vector<int>>& vis, int sr, int sc, int color, int initial, int dx[], int dy[]) {
        ...
        for(int i = 0 ; i < 4; i++) {
            ...
            if(nr >= 0 && nr < n && nc >= 0 && nc < m && vis[nr][nc] == 0 && image[nr][nc] 
                == initial) {
                image[nr][nc] = color;
                dfs(image, vis, sr, sc, color,initial, dx, dy);
            }
...
void prakhar() {   
    ...
    image = prakhar(image, sr, sc, color);
