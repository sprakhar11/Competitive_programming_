...
map<int, int> vtomap(vector<int> v) { map<int , int> mp; for(int i = 0 ; i < v.size() ; i++) {mp[v[i]]++;}  return mp;}

bool check_ans(int ans, map<int, int> mp, int mx_target) {
    ...
    // cout << mp[mx_target] << " " << same<<endl;

    if(mp[mx_target] >= same)
    {
        for(auto it:mp) {
            if(it.ff != mx_target) {
                diff++;
...
void prakhar() {   
    ...
    while(start <  end){
        ...
        // cout << mid << " checking this ans " << endl;

        if( check_ans(mid, mp, k)) {
            // cout << "true " << endl;
            start = mid + 1;
        } else {
