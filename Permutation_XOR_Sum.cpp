...
string to_binary(int n , int bit) { string s = ""; for (int i = bit; i >= 0; i--) { int k = n >> i; if (k & 1) s = s + "1";else s = s + "0";}return s ; }
...
void updatesum(vi &v , int &sum,int val) 
{
...
{   
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if ( val <= pow(2 , k) )
        {
            string binn = to_binary(val);
            string nev = neg(binn);
            int nev_dec = to_dec(nev);
            if(v[nev_dec-1] == 1)
            {
                done[val-1] = 0 ;
                v[nev_dec-1] = 0;
