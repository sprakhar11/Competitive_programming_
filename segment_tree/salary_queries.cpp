...
void prakhar() {
    ...
    for (int i = 0; i < q; i++)
    {
		...
		if( c == '!')
		{
            ...
            int val_tmp = sgt.query(0, 0, mx-1, tmp, tmp);
            // cout << "val_tmp = " << val_tmp;
            sgt.update(0, 0, mx-1, tmp, val_tmp-1);
        //    cout <<  sgt.query(0, 0, mx-1, tmp, tmp);

            int val2 = sgt.query(0, 0, mx-1, b ,b);
            sgt.update(0, 0, mx-1, b, val2+1);

		}
		else{

            // cout << a << " " << b << endl;
            cout << sgt.query(0, 0, n-1, --a , --b ) << endl;
        }
